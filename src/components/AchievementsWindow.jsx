...
export default function AchievementsWindow({ unlockedIds, knownSecretIds = [], adminMode = false, onClose }) {
  ...
  const visibleSecrets = adminMode
    ? typedSecrets
    : typedSecrets.filter((secret) => knownSecrets.has(secret.id));
  const showSecrets = adminMode || visibleSecrets.length > 0;
                ...
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <span className="text-[0.65rem] font-black uppercase tracking-[0.22em] text-zinc-400">
                      {discovered ? "Discovered" : "Admin Preview"}
                    </span>
                  </div>
                  <p className="mt-3 text-sm font-black text-white">{secret.label}</p>
